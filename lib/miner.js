...
process.on('message', async function(message) {
    ...
    sha256.update(JSON.stringify(blockNoTimestamp));
    const latestHash = sha256.digest('hex');
    console.log(`Start mining ${latestHash} ${message.difficulty}`);
...
async function validate(block_hash, block_nonce, difficulty) {
    ...
    //   console.log("block_hash is: " + block_hash);
    //   console.log("nonce is: " + block_nonce);
    //   console.log("validation result: " + result);
    //   console.log("expected is: " + difficulty);
    // }
 ...
 async function mining(block, difficulty) {
    ...
    // console.log("just after validate with nonce: " + nonce);
    // console.log(v);
    if (v) { break }
